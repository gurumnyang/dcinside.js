// Warning: 실행 시 실제 게시글이 삭제됩니다. 대상 글 번호와 갤러리를 반드시 확인하세요.
// 실행 예 (로그인):
//   DCINSIDE_CODE=식별코드 DCINSIDE_PASSWORD=비밀번호 node tests/practice/mobile.post.delete.practice.js chatgpt 123456 --login
// 실행 예 (비로그인):
//   node tests/practice/mobile.post.delete.practice.js chatgpt 123456 --guest --nickname 닉네임 --password 글비밀번호

const api = require('../../');

function parseArgs(argv) {
  const [, , galleryId, postId, ...rest] = argv;
  const options = { galleryId, postId, mode: 'login', nickname: undefined, password: undefined };
...
async function run() {
  const { galleryId, postId, mode, nickname, password } = parseArgs(process.argv);
  if (!galleryId || !postId) {
    console.error('사용법: node tests/practice/mobile.post.delete.practice.js <galleryId> <postId> [--login|--guest ...]');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    if (mode === 'login') {
      ...
      jar = login.jar;
      console.log('[정보] 로그인 성공. 게시글을 삭제합니다.');
    } else {
