...
describe('scraper 주요 기능', () => {
    test('scrapeBoardPages: 게시글 번호 배열 반환', async () => {
        const posts = await scrapeBoardPages(1, 1, 'chatgpt', { boardType: 'all' });
    ...
    test('scrapeBoardPages: 잘못된 갤러리 ID 입력 시 빈 배열 반환', async () => {
        const posts = await scrapeBoardPages(1, 1, '존재하지않는갤러리', { boardType: 'all' });
        expect(Array.isArray(posts)).toBe(true);
    ...
    test('scrapeBoardPages: 페이지 범위가 0 이하일 때 빈 배열 반환', async () => {
        const posts = await scrapeBoardPages(0, 0, 'chatgpt', { boardType: 'all' });
        expect(Array.isArray(posts)).toBe(true);
        expect(posts.length).toBe(0);
    });
